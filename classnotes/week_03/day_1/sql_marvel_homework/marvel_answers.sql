...
-- % is a wildcard, and we can use LIKE instead of =
DELETE FROM people WHERE name LIKE '%Craig%';
-- this second command would delete a row with the names:
-- Daniel Craig
-- Craig Morton
-- Craig David
-- blahblahCraiglala
...
UPDATE people SET name = 'Unwanted David' WHERE name = 'David';
-- or
...
-- or
UPDATE movies SET show_time = '18:30'
...
-- we can use lots of OR to have mutliple conditions in our WHERE clause
DELETE FROM movies
WHERE title = 'Thor'
OR title = 'Batman Begins'
OR show_time = '00:00'
OR title = 'Avengers Assemble';

-- We can delete all entrys that match IN a given set of values
DELETE FROM movies WHERE id IN (1, 2, 3);

-- We can delete all entrys with id BETWEEN two numbers
